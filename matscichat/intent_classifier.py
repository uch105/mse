...
class IntentClassifier:
    """Advanced intent classification for materials science queries"""
    ...
    # Materials database
    MATERIALS = [
        "steel", "aluminum", "aluminium", "copper", "diamond", "glass", 
        "plastic", "titanium", "magnesium", "iron", "graphene", "silicon", 
        "gold", "platinum", "nickel", "cnt", "carbon fiber", "stainless steel",
        "brass", "bronze", "zinc", "lead", "tin"
    ]
    
    # Applications/Environments
    APPLICATIONS = [
        "seawater", "sea water", "saltwater", "ocean", "marine",
        "aerospace", "automotive", "construction", "medical",
        "coastal", "underwater", "acidic", "alkaline", "high temperature",
        "cryogenic", "corrosive"
    ]
    ...
    def _train(self):
        ...
        self.clf = Pipeline([
            ...
            ("logreg", LogisticRegression(max_iter=200, solver="lbfgs", multi_class='ovr'))
        ])
    ...
    def extract_materials(self, query: str) -> List[str]:
        ...
        for material in sorted(self.MATERIALS, key=len, reverse=True):
            if material in query_lower and material not in found:
                found.append(material)
        
        return list(set(found))
    
    def extract_applications(self, query: str) -> List[str]:
        """Extract applications/environments from query"""
        query_lower = query.lower()
        found = []
        
        for app in self.APPLICATIONS:
            if app in query_lower:
                # Normalize sea water variants
                if app in ["seawater", "sea water", "saltwater", "ocean", "marine"]:
                    if "seawater" not in found:
                        found.append("seawater")
                else:
                    found.append(app)
    ...
    def detect_intents(self, query: str) -> List[str]:
        ...
        # Get primary intent from classifier
        proba = self.clf.predict_proba([query])[0]
        classes = self.clf.classes_
        
        # Add intents with confidence > 0.2
        for idx, prob in enumerate(proba):
            if prob > 0.2:
                intents.append(classes[idx])
        
        # Keyword-based intent detection for degradation/corrosion
        degradation_keywords = [
            'corrode', 'corrosion', 'rust', 'degrade', 'degradation',
            'oxidize', 'oxidation', 'faster', 'slower', 'rate',
            'weathering', 'deteriorate', 'decay'
        ]
        
        if any(keyword in query_lower for keyword in degradation_keywords):
            if 'degradation_forecast' not in intents:
                intents.append('degradation_forecast')
        
        # Comparison keywords
        comparison_keywords = [
            'compare', 'vs', 'versus', 'better', 'which', 'difference',
            'between', 'among'
        ]
        ...
        return intents if intents else [classes[proba.argmax()]]
    ...
    def analyze_query(self, query: str) -> Dict[str, Any]:
        ...
        applications = self.extract_applications(query)
        
        # Get confidence for primary intent
        proba = self.clf.predict_proba([query])[0]
